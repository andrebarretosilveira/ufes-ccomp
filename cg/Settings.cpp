...
bool Settings::read_xml(char* config_filepath)
{
    ...
    GLfloat obstacleHeight = obstacleElement->FloatAttribute("altura");
    obstacleHeight /= 1;
    ...
    for (XMLElement* circleElement = svg->FirstChildElement("circle"); circleElement != NULL; circleElement = circleElement->NextSiblingElement("circle")) {
        ...
        if(strcmp(fill, "blue") == 0) {
            ...
            this->outerLimit = new Cylinder(r, 300, transform, Color(0,1,1));
            this->outerLimit->diskOnTop = false;
        }
        else if(strcmp(fill, "white") == 0) {
            Transform transform = Transform(Vector3(x,y,0), Vector3(0,0,0), Vector3(1,1,1));
            this->innerLimit = new Cylinder(r, 100, transform, Color(1,1,1));
            this->innerLimit->diskOnTop = true;
        }
        else if(strcmp(fill, "green") == 0) {
            Transform transform = Transform(Vector3(x,y,0), Vector3(0,0,0), Vector3(1,1,1));
            // playerCircle = new Circle(id, r, transform, Color(0,1,0));
            playerSphere = new Sphere(r,transform,Color(0,1,0));
            this->player = new Player(playerSphere, playerSphere->transform, playerMoveSpeed, playerBulletSpeed, enemyFireFreq);
        }
        else if(strcmp(fill, "red") == 0) {
            Transform transform = Transform(Vector3(x,y,0), Vector3(0,0,0), Vector3(1,1,1));
            // Circle* circle = new Circle(id, r, transform, Color(1,0,0));
            Sphere* sphere = new Sphere(r, transform, Color(1,0,0));
            enemies.push_back(new Player(sphere, sphere->transform, enemyMoveSpeed, enemyBulletSpeed, enemyFireFreq));
        }
        else if(strcmp(fill, "black") == 0) {
            Transform transform = Transform(Vector3(x,y,0), Vector3(0,0,0), Vector3(1,1,1));
            Cylinder* cylinder = new Cylinder(r, obstacleHeight, transform, Color(0,0,0));
            cylinder->diskOnTop = true;
            cylinder->inside = false;
            obstacles.push_back(new Obstacle(cylinder, true));
        }
    }
