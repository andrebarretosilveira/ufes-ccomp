...
// Constructor
Bullet::Bullet(Vector3 position, Vector3 moveDirection, GLfloat moveSpeed, GLfloat radius, Color color)
	  ...
	  moveDirection(moveDirection), moveSpeed(moveSpeed), radius(radius)
{
	...
	// 	Color(0,0,0));
	this->shape = new Sphere(radius, Transform(Vector3(0,0,0), Vector3(0,0,0), Vector3(1,1,1)),
		color);
...
void Bullet::move()
{
	...
	transform.position.z += moveDirection.z * moveSpeed * Time::deltaTime.count() * 1000;
}

bool Bullet::isTouching(Player* player) {
	return this->shape->isTouchingSphere(player->transform.position, player->getOrgRadius());
}
