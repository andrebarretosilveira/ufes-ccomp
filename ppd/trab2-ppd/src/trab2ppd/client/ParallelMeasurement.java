...
import trab2ppd.master.Attacker;
import trab2ppd.utils.FileAccess;
import trab2ppd.utils.Guess;

/**
 * Measures the time taken to do a dictionary attack on a random byte[] using a
 * remote server.
 */
public class ParallelMeasurement {
	...
	/**
	 * Assigns a new random byte[] of a specific size to the text to be decoded.
	 * 
	 * @param size
	 *            is quantity of elements in the byte[].
	 */
	...
	/**
	 * Request attacks multiple times to the server and returns the average of the
	 * time taken to process all attack.
	 * 
	 * @param bytesNumber
	 *            is the size of the random byte[] to be attacked.
	 * @param repetitions
	 *            is the number of attacks to be considered.
	 */
	public double attackAverageSecondsTaken(int bytesNumber, int repetitions) {
		...
		try {
			...
			System.out.println(
					"Error: Server connection has failed. Server either ended connection or it could not be reachead.");
	...
	/**
	 * Send a attack request to the server, and assign the found guesses in the
	 * guesses attribute.
	 * 
	...
	/**
	 * Constructs a new ParallelMeasurement and uses it to measure the time taken to
	 * execute a remote attack for different sizes of text.
	 */
