...
public class Client {
	...
	public static void main(String[] args) {
		...
		if (args.length < 3) {
			System.out.println(
					"Invalid input\nCorrect usage:\n   "
					+ "java trab2ppd.client.Client <server adress> <input text pathname> <known word> <(optional)randombyte array size>");
			return;
		}
		...
		try {
			...
			try {
				...
				System.out.println(
						"IOException: saving Random Bytes.cipher has failed. Processing will continue nonetheles.");
			}
	 ...
	 * 
	 * @param cipherText
	 *            is the encrypted text to be decoded.
	 * @param knowntext
	 *            is the word from the original text previously known.
	 * @throws RemoteException
	 *             failed to send the request.
	 */
	public void orderAttack(byte[] cipherText, byte[] knowntext) throws RemoteException {
