...
# Runs the program in read-only mode, then delete the input files and run write-only.
cd $srcDir
ant run-read-only > /dev/null 2> /dev/null
rm *.csv *.txt > /dev/null 2> /dev/null
ant run-write-only > /dev/null 2> /dev/null
exitcode=$?
cd - > /dev/null
